...
test_image() {
    ...
    WAILS_VERSION=$(docker run --rm --entrypoint="" "$FULL_IMAGE_NAME" wails3 version)
...
setup_distrobox() {
    ...
    distrobox create wails-development --image "$FULL_IMAGE_NAME"
    
    if [ $? -eq 0 ]; then
        print_success "Distrobox container created successfully"
        print_info "You can now enter the container with: distrobox enter wails-development"
    else
